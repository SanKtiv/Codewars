...
//For example, '02078834982' has somehow become 'efRFS:)0207ERGQREG88349F82!' and
// there are lots more lines that we need to check.
//The Task
//Given a string, you must decide whether or not it contains a valid phone number.
// If it does, return the corrected phone number as a string ie. '02078834982' with
// no whitespace or special characters, else return "Not a phone number".

//SOLUTIONS:

function isItANum (str) {
    let reg = /\d+/g;
    let numPhone = str.match(reg).join('');
    return numPhone.length == 11 && numPhone[0] == 0
        ? numPhone
        : "Not a phone number";
}